    ...
    conditionalPanel(
      ...
      div(
        ...
        wellPanel(
          fluidRow(
            column(width = 3, 
                   selectInput(ns("map_empid_column"), "社員番号に対応する列:", choices = c("未選択" = ""))
                   ),
            column(width = 3, 
                   selectInput(ns("map_age_column"), "年齢に対応する列:", choices = c("未選択" = "")),
                   selectInput(ns("map_gender_column"), "性別に対応する列:", choices = c("未選択" = ""))
                   ),
            column(width = 3, 
                   selectInput(ns("map_name_column"), "氏名に対応する列:", choices = c("未選択" = ""))
                   ),
            column(width = 3, 
                   selectInput(ns("map_dept1_column"), "部署(大分類)に対応する列:", choices = c("未選択" = "")),
                   selectInput(ns("map_dept2_column"), "部署(中分類)に対応する列:", choices = c("未選択" = ""))
            ),
    ...
    ## ステップ4: データ加工の終了と分析画面への ----------------------
      conditionalPanel(
        ...
        div(
          ...
          p("データの変換が完了いたしました。"),
          p("変換されたデータは、以下のボタンからダウンロードいただいて、個人結果表示ツールでご利用ください。"),
          fluidRow(
            column(width = 6,
                   style = "margin-top: 25px;",
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$csv_file_input, {
      ...
      tryCatch({
        ...
        updateSelectInput(session, "map_name_column", choices = header_choices, selected = rv$column_map_name)
    ...
    observeEvent(input$col_map_config_load_input, {
      ...
      tryCatch({
        ...
        rv$column_map_name   <- config$basic_attributes$name %||% ""
    ...
    output$col_map_config_save_button <- downloadHandler(
      ...
      content = function(file) {
        ...
        rv$column_map_name   <- input$map_name_column %||% ""
    ...
    observeEvent(input$finish_setup_button, { 
      ...
      #2. 列名マッピングの適用
       name_map <- c(rv$column_map_empid , 
                     rv$column_map_age  , 
                     rv$column_map_name  , 
                     rv$column_map_gender, 
                     rv$column_map_dept1, 
                     rv$column_map_dept2 , 
                     rv$column_map_nbjsq ) |> 
         as.character() |> 
         setNames(c("empid","age","name","gender","dept1","dept2",str_c("q",1:80)))
    ...
    observeEvent(input$read_processed_csv_button, {
      ...
      if(is.null(input$processed_file_input)){
        ...
        #列名のチェック
        required_columns <- c(
          "empid", "age", "name","age_kubun", "gender", "dept1", "dept2",
          str_c("q",1:80),
          "A", "B","C","is_hs",
          str_c("q", 1:80, "_score"),
