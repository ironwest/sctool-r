...
analysis_table_module_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    display_data <- eventReactive(input$update_table_button, {
      ...
      #高ストレス者の人数と割合を計算する
      group_vars <- c("dept1","dept2")
      hyouhs <- current_data |> 
        group_by(across(all_of(group_vars))) |> 
        summarise(
          n = n(),
          miss_n = sum(is.na(is_hs)),
          hs_n = sum(is_hs, na.rm=TRUE),
          hs_p = hs_n/n
        )
      
      hyou <- hyou |> 
        left_join(hyouhs, by=group_vars)
    ...
    output$summary_table <- renderReactable({
     ...
     #コラムグループの設定
      ...
      column_setting_list$dept1 <- horizontal_col_def
      column_setting_list$dept2 <- horizontal_col_def
      ...
      hyou <- hyou |> relocate(!!!rlang::syms(column_order))
      　
        reactable(
